"use client"
import BookingModal from '@/app/admin/_modal/BookingModal'
...
const BookCarPage = ({params} : {params : Promise<{id : number}>}) => {
  const {id} = use(params)
    ...
    const [bookingOpen, setBokingOpen] = useState<boolean>(false)
   if (!car) {
  return (
  ...
  return (
    <>
    <Navbar />
    <div className='max-w-7xl mx-auto px-4 py-6 overflow-hidden mt-20'>
  ...
  <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
    ...
    <div className='flex flex-col gap-6'>
      ...
      <div className='flex items-end my-auto'>
        <Button onClick={() => {
          setBokingOpen(true)
        }} className={cn('px-5')}>Book Now</Button>
      </div>
    </div>
  </div>
...
{bookingOpen && <BookingModal car={car} onClose = {() => setBokingOpen(false)}/>}
</>
  )
}
